...
### 3. No panic hook for terminal restoration

**Location:** `src/bin/mergers.rs`

The terminal is set up with `enable_raw_mode`, `EnterAlternateScreen`, and `EnableMouseCapture`, but no `std::panic::set_hook` is installed. If any UI code panics (and there are multiple `.unwrap()` calls in render paths like `cherry_pick.rs:193`, `conflict_resolution.rs:313`), the terminal is left in raw mode with the alternate screen active. The user would need to run `reset` or `stty sane` to restore their terminal. This is a well-known gap in ratatui applications.

---

## High Severity Findings

### 4. Double lock acquisition causes self-deadlock in `run()`
...
### 5. `continue_merge` never calls `git cherry-pick --continue`
...
### 7. No API retry logic (parameter is silently discarded)
...
### 8. TOCTOU race condition in lock acquisition
...
### 9. State not persisted during cherry-pick loop
...
### 10. No rate limit handling for Azure DevOps API
...
### 11. Work item batch size not chunked
...
### 14. `get_commit_info` uses `|` as delimiter
...
### 17. `all_work_items_terminal` carries dual semantics
...
| # | Finding | Location |
|---|---------|----------|
| 18 | `force_remove_worktree`, `force_delete_branch`, `abort_cherry_pick` silently discard all errors | `git.rs:418, 444, 664` |
| 19 | Inconsistent use of `git_command()` helper (~50% bypass it) | `git.rs` |
| 20 | `.to_str().unwrap()` on paths can panic on non-UTF-8 | `git.rs:234, 300` |
| 21 | `parse_patch_branch` splits on wrong hyphen for multi-hyphen branch names | `git.rs:1111` |
| 22 | `Throttler::new(0)` deadlocks (no input validation) | `utils/throttle.rs` |
| 23 | `Semaphore::acquire()` result silently ignored | `utils/throttle.rs:26` |
| 24 | `_max_concurrent_processing` parameter accepted but unused | `utils/throttle.rs:43` |
| 25 | `MergeArgs`, `MigrateArgs`, `CleanupArgs` missing `Debug` derive | `models.rs:319, 342, 357` |
| 26 | `CherryPickStatus`/`CleanupStatus` missing `PartialEq`/`Display` | `models.rs:1414, 1469` |
| 27 | PAT field is `pub` with no Debug redaction | `models.rs:797` |
| 28 | `categorize_prs` returns `Result` but never errors | `migration.rs:190` |
| 29 | Unnecessary `.clone()` on `unsure_details` | `migration.rs:250` |
| 30 | `Loading` and `Setup` phases are dead code | `MergePhase` enum |
| 31 | `run_hooks` config carried through all layers but never implemented | `cherry_pick.rs:195-196` |
| 32 | `abort()` returns exit code 0 (indistinguishable from success) | `non_interactive.rs` |
| 33 | No stale state file cleanup / no orphaned `.json.tmp` cleanup | State directory |
| 34 | Misleading backward-compat constructors accept `pool_idle_timeout_secs`/`max_retries` but ignore them | `api/client.rs` |
| 35 | `get_config_path()` panics with `.expect()` when home directory unavailable | `config.rs:335` |
| 36 | No validation of numeric config values (0 allowed for concurrency limits) | `config.rs` |
| 37 | Version input accepts arbitrary strings used in branch names/paths | `ui/state/default/version_input.rs:79` |
| 38 | Panicking `.unwrap()` calls in render paths (version, repo_path, array index) | `ui/state/default/cherry_pick.rs:193`, `conflict_resolution.rs:313` |
| 39 | Config directory created as side effect of read-only `load_from_file()` | `config.rs:329-351` |
| 40 | `save_ui_settings` drops TOML comments and unknown fields during round-trip | `config.rs:458-486` |
...
1. **No E2E tests for non-interactive merge workflow.** `run()`, `continue_merge()`, `abort()`, `complete()`, and `status()` are not tested as integrated flows. This is why the double-lock bug (#4) and missing cherry-pick continue (#5) went undetected.
